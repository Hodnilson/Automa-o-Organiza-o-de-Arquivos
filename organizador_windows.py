#!/usr/bin/env python3
"""
Organizador Automático de Arquivos - Versão Windows (.exe)
Interface Gráfica moderna com tkinter + tema nativo do Windows.
"""
...
# Ícones para cada categoria (emojis compatíveis com Windows)
ICONES = {
    "Documentos": "📄",
    "Imagens": "🖼️",
    "Vídeos": "🎬",
...
def construir_mapa_extensao() -> dict[str, str]:
    """Inverte o dicionário CATEGORIAS: extensão → categoria."""
    mapa: dict[str, str] = {}
...
def organizar_pasta_logica(caminho_pasta: Path, progress_callback=None, log_callback=None) -> dict:
    """Lógica principal: percorre a pasta e move os arquivos para subpastas."""
    ...
    if not arquivos:
        return {"total_movidos": 0, "total_erros": 0, "contagem": {}, "logs": []}
    ...
    for i, arquivo in enumerate(arquivos):
        ...
        try:
            ...
            msg = f"  ✅ {arquivo.name}  →  {categoria}/"
            logs.append(msg)
            if log_callback:
                log_callback(msg)
        except PermissionError:
            msg = f"  🔒 Sem permissão: {arquivo.name}"
            logs.append(msg)
            if log_callback:
                log_callback(msg)
            total_erros += 1
        except Exception as e:
            msg = f"  ❌ Erro: {arquivo.name} ({e})"
            logs.append(msg)
            if log_callback:
                log_callback(msg)
            total_erros += 1
...
# ─────────────────────────────────────────────────────────────
# INTERFACE GRÁFICA (GUI) — Versão Windows Aprimorada
# ─────────────────────────────────────────────────────────────
class OrganizadorApp(tk.Tk):
    ...
    def _criar_widgets(self):
        ...
        tk.Label(
            header,
            text="🗂️  Organizador Automático de Arquivos",
        ...
        tk.Label(
            info_frame,
            text="Categorias:   📄 Documentos  │  🖼️ Imagens  │  🎬 Vídeos  │  🎵 Música  │  📦 Outros",
            font=("Segoe UI", 9),
            bg=self.COR_PAINEL,
    ...
    def selecionar_pasta(self):
        ...
        # No Windows, usa a pasta do usuário como padrão
        ...
        if pasta:
            ...
            self.lbl_status.config(text=f"Pasta selecionada: {pasta}", fg=self.COR_TEXTO)
    ...
    def iniciar_organizacao(self):
        """Inicia a organização em uma thread separada."""
        ...
        def executar():
            ...
            def mostrar_resultado():
                ...
                if resultados["total_movidos"] == 0:
                    ...
                    msg = f"✅ {resultados['total_movidos']} arquivo(s) organizados com sucesso!\n\n"
                    for cat, count in sorted(resultados["contagem"].items()):
                        icone = ICONES.get(cat, "📦")
                        msg += f"  {icone}  {cat}: {count} arquivo(s)\n"

                    if resultados["total_erros"] > 0:
                        msg += f"\n❌ {resultados['total_erros']} arquivo(s) com erro."

                    self.lbl_status.config(
                        text=f"Concluído! {resultados['total_movidos']} arquivo(s) organizados.",
                        fg=self.COR_SUCESSO
                    )
